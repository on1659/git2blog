...
| `NAVER_CLIENT_ID` | 네이버 블로그 API | 네이버 사용 시 |
| `NAVER_CLIENT_SECRET` | 네이버 블로그 API | 네이버 사용 시 |
| `TISTORY_CLIENT_ID` | Tistory API | Tistory 사용 시 |
| `TISTORY_CLIENT_SECRET` | Tistory API | Tistory 사용 시 |
...
│   ├── app/
│   │   ├── page.tsx                  # 대시보드
│   │   ├── write/page.tsx            # 새 글 작성
│   │   ├── edit/[id]/page.tsx        # 에디터
│   │   ├── settings/page.tsx         # 플랫폼 설정
...
│   └── lib/
│       ├── db/
│       │   ├── index.ts              # SQLite 연결 + globalThis 캐싱
│       │   └── schema.ts             # Drizzle 스키마
│       └── platforms/
│           ├── types.ts              # BlogPlatform 인터페이스
│           ├── registry.ts           # 플러그인 등록
│           ├── hashnode.ts
│           ├── devto.ts
│           ├── medium.ts
...
- [기획서](SPEC.md) — 시스템 아키텍처, DB 스키마, API 명세
- [디자인 시스템](DESIGN.md) — 색상, 타이포그래피, 컴포넌트 가이드
