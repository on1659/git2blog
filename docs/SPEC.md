...
CLI 기반 Claude Code 워크플로우(GitHub 커밋 분석 → 블로그 글 생성 → 발행)를 웹 애플리케이션으로 확장.
브라우저에서 글 생성, 편집, 멀티 플랫폼 동시 발행까지 한 번에 처리한다.

### 핵심 가치

- **자동화**: GitHub URL만 넣으면 AI가 커밋 분석 → 글 제안 → KO/EN 동시 생성
- **멀티 플랫폼**: Hashnode, DEV.to, Medium, 네이버 블로그, Tistory 등 여러 블로그에 한 번에 발행
...
| platform_id | TEXT | 플랫폼 식별자 ('hashnode', 'devto', 'medium', 'naver', 'tistory') |
...
- 통계 카드: 전체 글, 발행됨, 초안
- 최근 글 목록: 제목, KO/EN 뱃지, 플랫폼 뱃지, 상태별 필터링
...
- Publish Modal: 플랫폼 선택 (언어별), 초안 옵션, 결과 표시
...
| naver | 네이버 블로그 | REST | clientId, clientSecret |
| tistory | Tistory | REST | clientId, clientSecret |
...
예: blog_01_ko_vapecompare-naver-api.md
    blog_01_en_vapecompare-naver-api.md
```
